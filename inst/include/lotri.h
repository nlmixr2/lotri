...
typedef int (*lotriNearPDc_type)(double *, double *, int, int, int, int, int,
                                 double, double, double, int, int);
extern lotriNearPDc_type lotriNearPDc;

typedef SEXP (*lotriNearPDsexp_type)(SEXP, SEXP, SEXP, SEXP, SEXP, SEXP, SEXP,
                                     SEXP, SEXP, SEXP);
extern lotriNearPDsexp_type lotriNearPDsexp;

static inline void iniLotriPtr0(SEXP ptrLst) {
  // SEXP lotriLstToMatPtr =
  // PROTECT(R_MakeExternalPtrFn((DL_FUNC)&_lotriLstToMat, R_NilValue,
  // R_NilValue)); pro++;
  if (lotriLstToMat == NULL) {
    lotriLstToMat =
        (lotriLstToMat_type)R_ExternalPtrAddrFn(VECTOR_ELT(ptrLst, 0));
    ...
    lotriAllNames =
        (lotriAllNames_type)R_ExternalPtrAddrFn(VECTOR_ELT(ptrLst, 3));
    lotriGetBounds =
        (lotriGetBounds_type)R_ExternalPtrAddrFn(VECTOR_ELT(ptrLst, 4));
    ...
    lotriNearPDc =
        (lotriNearPDc_type)R_ExternalPtrAddrFn(VECTOR_ELT(ptrLst, 8));
    lotriNearPDsexp =
        (lotriNearPDsexp_type)R_ExternalPtrAddrFn(VECTOR_ELT(ptrLst, 9));
  }
}
...
#define lotriNearPDarmaSetup                                                   \
  static inline bool lotriNearPDarma(                                          \
      arma::mat &ret, arma::mat x, bool keepDiag = true, bool do2eigen = true, \
      bool doDykstra = true, bool only_values = false, double eig_tol = 1e-6,  \
      double conv_tol = 1e-7, double posd_tol = 1e-8, int maxit = 1000,        \
      bool trace = false) {                                                    \
    ret = arma::mat(x.n_rows, x.n_cols);                                       \
    return lotriNearPDc(ret.memptr(), x.memptr(), x.n_rows, keepDiag,          \
                        do2eigen, doDykstra, only_values, eig_tol, conv_tol,   \
                        posd_tol, maxit, trace);                               \
  }
