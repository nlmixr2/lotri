test_that("same() repeats the preceding block", {
  ...
  expect_equal(dimnames(.m)[[1]], c("iov.cl1", "iov.v1", "iov.cl2", "iov.v2", "iov.cl3", "iov.v3"))
...
test_that("the condition column names the mirrored element", {
  ...
  expect_equal(.df$condition, c("id", "id", "id", "id:same:iov.cl1", "id:same:iov.cl1:iov.v1", "id:same:iov.v1"))
...
test_that("same() round trips through the data frame and the expression", {
  ...
  expect_equal(attr(lotri::as.lotri(.df), "lotriSame"), c(0L, 0L, 0L, 2L, 2L, 4L, 4L))
...
test_that("same() composes with the cnd(same=n) repeat property", {
  ...
  expect_equal(attr(.mm, "lotriSame"), rep(c(0L, 0L, 2L, 2L), 3))
})
...
test_that("the exported helpers read the condition column", {
  ...
  expect_equal(lotri::lotriIsSame(.df$condition), c(FALSE, FALSE, FALSE, FALSE, TRUE, TRUE, TRUE))
...
test_that("lotriSameBreak() unlinks only the block that was touched", {
  ...
  expect_equal(lotri::lotriSameBreak(.df, character(0))$condition, .df$condition)
  ...
  for (.e in c("a2", "b2", "a1", "b1")) {
    ...
    expect_equal(.b$condition[.b$name %in% c("a2", "b2", "(a2,b2)")], rep("id", 3))
...
test_that("same() error paths", {
  ...
  expect_error(lotri::lotri(a + b ~ c(1, 0.1, 1), c1 + d1 ~ same()), "in the same '{}' block", fixed = TRUE)
  ...
  expect_error(
    ...
    "a 'cov' function cannot be used with 'same()'",
    fixed = TRUE
  )
})
...
test_that("as.expression() only writes same() when it really re-parses", {
  ...
  expect_false(any(grepl("same()", as.character(as.expression(.bad)), fixed = TRUE)))
  expect_equal(unclass(eval(as.expression(.bad))), unclass(.bad), ignore_attr = TRUE)
  ...
  expect_equal(unclass(eval(as.expression(.chain))), unclass(.chain), ignore_attr = TRUE)

  ## the ordinary case is unaffected
  expect_true(any(grepl("c1 + d1 ~ same()", as.character(as.expression(.m)), fixed = TRUE)))
})
...
test_that("a repeated block under a condition names its master correctly", {
  ...
  .df <- as.data.frame(.m)
  expect_equal(.df$condition, c(NA, "id", "occ", "occ", "occ", "occ:same:a", "occ:same:a:b", "occ:same:b"))
  expect_equal(as.data.frame(lotri::as.lotri(.df)), .df)
  expect_equal(attr(lotri::as.lotri(.df)$occ, "lotriSame"), c(0L, 0L, 2L, 2L))
})
...
test_that("a label same() cannot carry blocks the same() shorthand", {
  ...
  expect_false(any(grepl("same()", as.character(as.expression(.bad)), fixed = TRUE)))
  expect_equal(attr(eval(as.expression(.bad)), "lotriLabels"), c(NA, NA, "first", NA))
})
...
test_that("a repeated block survives lotriSep() nesting", {
  ...
  expect_equal(attr(.mm, "lotriSame"), c(0L, rep(c(0L, 0L, 2L, 2L), 2)))
})
...
test_that("the condition base follows the `default` argument", {
  ...
  expect_equal(
    as.data.frame(.m, default = "occ")$condition,
    c("occ", "occ", "occ", "occ:same:a", "occ:same:a:b", "occ:same:b")
  )
})
...
test_that("same() is not emitted across an intervening independent block", {
  ...
  expect_false(any(grepl("same()", as.character(as.expression(.l)), fixed = TRUE)))
  ...
  expect_true(any(grepl("e1 + f1 ~ same()", as.character(as.expression(.chain)), fixed = TRUE)))
})
...
test_that("a block sliced away from its master degrades gracefully", {
  ...
  expect_false(any(grepl("repeat", capture.output(print(.b)), fixed = TRUE)))
})
...
test_that("blocks that merely agree to a tolerance are not collapsed", {
  ...
  expect_false(any(grepl("same()", as.character(as.expression(.near)), fixed = TRUE)))
...
test_that("the whole-omega prior shorthand skips repeated blocks", {
  ...
  expect_equal(attr(.m, "lotriPriors"), c("invWishart(4)", NA, NA, NA))
  expect_equal(as.data.frame(lotri::as.lotri(as.data.frame(.m))), as.data.frame(.m))
  expect_equal(as.data.frame(eval(as.expression(.m))), as.data.frame(.m))
...
test_that("same() repeats the master after any transformation", {
  ...
  for (.e in list(
    ...
    .m <- eval(.e)
    expect_equal(attr(.m, "lotriSame"), c(0L, 0L, 2L, 2L))
    expect_equal(unclass(.m)[3:4, 3:4], unclass(.m)[1:2, 1:2], ignore_attr = TRUE)
  }
})
...
test_that("same() is not emitted past a block that is itself written out", {
  ...
  expect_equal(diag(eval(as.expression(.x))), c(a = 1, b = 2, c1 = 99, d1 = 88, e1 = 1, f1 = 2))
...
test_that("labels and fixed flags are judged by the shared view", {
  ...
  ## other two dropped
  expect_false(any(grepl(" repeat ", capture.output(print(.lab)), fixed = TRUE)))
})
...
test_that("an offset that no longer describes a mirror is dropped", {
  ...
  expect_equal(diag(unclass(lotri::as.lotri(as.data.frame(.dropped)))), c(z = 9, b = 1))
...
test_that("a block with mixed offsets does not index out of range", {
  ...
  .b <- lotri::lotriMatInv(lotri::lotriEst(lotri::as.lotri(.df), drop = TRUE))[[2]]
...
test_that("two repeated blocks with the same offset stay separate", {
  ...
  expect_equal(attr(.f, "lotriSame"), c(0L, 0L, 2L, 2L, 0L, 0L, 2L, 2L))
  expect_equal(
    as.data.frame(.f)$condition,
    c(
      "id",
      "id",
      "id",
...
test_that("a valid repetition survives an invalid one next to it", {
  ...
  expect_false(any(lotri::lotriIsSame(.df$condition[
    .df$name %in%
      c("g1", "h1")
  ])))
  expect_equal(as.data.frame(lotri::as.lotri(.df)), .df)
  ...
  expect_equal(as.data.frame(eval(as.expression(lotri::as.lotri(.df2)))), .df2)
})
...
test_that("a repeated block with a structural zero round trips", {
  ...
  .e <- as.character(as.expression(.m))
  expect_true(any(grepl("c1 + d1 ~ same()", .e, fixed = TRUE)))
  expect_equal(attr(eval(as.expression(.m)), "lotriSame"), c(0L, 0L, 2L, 2L))
  expect_equal(as.data.frame(eval(as.expression(.m))), as.data.frame(.m))
  ...
  expect_equal(attr(eval(as.expression(.m3)), "lotriSame"), c(0L, 0L, 0L, 3L, 3L, 3L))
...
test_that("a repeated block must be separated from the rest of the matrix", {
  ...
  ## the 3x3 stays whole ...
  expect_equal(vapply(.lotriSameSplit(.l), function(x) dim(x)[1], integer(1)), c(3L, 2L))
...
test_that("a forced block boundary may not cut a covariance that spans it", {
  ...
  expect_equal(vapply(.lotriSameSplit(.l), function(x) dim(x)[1], integer(1)), c(1L, 3L))
...
test_that("a valid family survives a bogus one in the same run", {
  ...
  .df2 <- as.data.frame(.l)
  expect_equal(.df2$condition, c("id", "id:same:x1", "id:same:x1", "id:same:x1"))
  expect_equal(attr(eval(as.expression(.l)), "lotriSame"), c(0L, 1L, 2L, 3L))
  expect_equal(as.data.frame(eval(as.expression(.l))), .df2)
  expect_true(any(grepl("x3 repeat x1", capture.output(print(.l)), fixed = TRUE)))
})
...
test_that("the data frame cannot encode a linkage same() cannot write", {
  ...
  expect_equal(unclass(eval(as.expression(.l))), unclass(.l), ignore_attr = TRUE)
  ...
  expect_equal(as.data.frame(eval(as.expression(.chain))), as.data.frame(.chain))
})
...
test_that("a NONMEM style chain naming the preceding copy is normalised", {
  ...
  expect_equal(
    as.data.frame(.l)$condition,
    c("id", "id", "id", "id:same:a", "id:same:a:b", "id:same:b", "id:same:a", "id:same:a:b", "id:same:b")
  )
  expect_equal(as.data.frame(lotri::as.lotri(as.data.frame(.l))), as.data.frame(.l))
...
test_that("every consumer agrees on the linkage, over random matrices", {
  ...
  for (.it in seq_len(.iter)) {
    ...
    expect_equal(unclass(.viaEx), unclass(.x), ignore_attr = TRUE, info = .info)
    expect_equal(unclass(.viaDf)[seq_len(.n), seq_len(.n)], unclass(.x), ignore_attr = TRUE, info = .info)
    ...
    expect_equal(
      if (is.null(.oEx)) integer(.n) else as.integer(.oEx),
...
test_that("rcm() says so when it drops a repetition", {
  ...
  expect_warning(lotri::rcm(unclass(.m)), "drops the 'same()' repetition", fixed = TRUE)
...
test_that("a hand set double offset vector is carried by lotriMat()", {
  ...
  expect_equal(attr(lotri::lotriMat(list(.d)), "lotriSame"), c(0L, 0L, 2L, 2L))
})
...
test_that("a non-integral offset is not truncated into a repetition", {
  ...
  expect_equal(attr(lotri::lotriMat(list(.d)), "lotriSame"), c(0L, 0L, 0L, 0L))
})
...
test_that("random programs parse to the matrix a reference model predicts", {
  ...
  for (.it in seq_len(.iter)) {
    ...
    for (.b in seq_len(sample(1:3, 1))) {
      ...
      if (.useSame) {
        ...
        .lines <- c(
          ...
          switch(
            ...
            plus = paste0(paste(.nm, collapse = " + "), " ~ c(", paste(.lower(.m), collapse = ", "), ")"),
            line = vapply(
              seq_len(.k),
              function(.i) {
                paste0(.nm[.i], " ~ c(", paste(.m[.i, seq_len(.i)], collapse = ", "), ")")
              },
              character(1),
              USE.NAMES = FALSE
            )
      ...
      .sameOf <- c(.sameOf, if (.useSame) length(.blocks) - 1L else 0L)
      .nmOf[[length(.blocks)]] <- .nm
      .blockOf <- c(.blockOf, rep(length(.blocks), length(.lines) - .nLinesBefore))
      .nms <- c(.nms, .nm)
    }
    ...
    for (.l in unique(.lvl)) {
      ...
      expect_equal(.gm[seq_len(.n), seq_len(.n), drop = FALSE], .exp, ignore_attr = TRUE, info = .txt)
    }
  }
})
...
test_that("a block declared without a condition stays at the default level", {
  ...
  expect_equal(dimnames(unclass(.s$occ))[[1]], c("c1", "d1", "e1", "f1"))
...
test_that("same() finds its level across an intervening other level", {
  ...
  expect_equal(attr(.m$occ, "lotriSame"), c(0L, 0L, 2L, 2L))
  expect_equal(unclass(.m$occ)[3:4, 3:4], unclass(.m$occ)[1:2, 1:2], ignore_attr = TRUE)
...
test_that("an unconditioned same() does not inherit a stale level marker", {
  ...
  expect_equal(dimnames(unclass(.m$id))[[1]], c("z", "w", "u", "v", "x"))
  ...
  expect_equal(dimnames(unclass(.c$occ))[[1]], c("a", "b", "c1", "d1"))
})
