test_that("as.expression handling; lhs of theta parameters", {
  ...
  expect_equal(.lotriExpressionLinesFromDf1(x1), list(quote(tka <- c(0, 0.45, 1)), quote(label("Log Ka"))))

  x1$backTransform <- "exp"
  expect_equal(.lotriBackTransformFromDf1(x1), list(quote(backTransform("exp"))))

  expect_equal(
    .lotriExpressionLinesFromDf1(x1),
    list(quote(tka <- c(0, 0.45, 1)), quote(backTransform("exp")), quote(label("Log Ka")))
  )
  ...
  expect_equal(.lotriExpressionLinesFromDf1(x1), list(quote(tka <- c(0, 0.45, 1)), quote(backTransform("exp"))))

  x1$backTransform <- NA_character_
  expect_equal(.lotriExpressionLinesFromDf1(x1), list(quote(tka <- c(0, 0.45, 1))))

  expect_equal(
    .lotriGetPopLinesFromDf(xdf),
    list(
      quote(tka <- 0.45),
      quote(label("Log Ka")),
      quote(tcl <- 1),
  ...
  expect_equal(.lotriGetEtaMatEltPlusForm(x1), list(quote(et5 ~ 1), quote(et2 + et3 ~ c(1, 2, 3)), quote(et1 ~ fix(3))))
  ...
  expect_equal(.lotriGetEtaMatEltPlusForm(fix2), list(quote(m + n ~ c(2, 0.5, 1)), quote(f + g ~ fix(1, 0.5, 1) | occ)))

  expect_equal(
    .lotriGetEtaLineForm(fix2),
    list(quote(m ~ 2), quote(n ~ c(0.5, 1)), quote(f ~ fix(1) | occ), quote(g ~ fix(0.5, 1) | occ))
  )
  ...
  expect_equal(
    lotriDataFrameToLotriExpression(df, useIni = TRUE),
    quote(ini({
                 tka <- 0.45
                 label("Log Ka")
                 tcl <- 1
