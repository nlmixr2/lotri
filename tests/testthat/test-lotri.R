...
test_that(".maxNu", {
  ...
  expect_equal(lotriMaxNu(omega9), 0)
})

test_that("isLotri C", {
  expect_equal(isLotri(omega9), TRUE)
  expect_equal(isLotri(omega), TRUE)

  omega9[[2]] <- 3
  expect_equal(isLotri(omega9), FALSE)

  omega9[[2]] <- matrix(3)
  expect_equal(isLotri(omega9), FALSE)

  expect_equal(isLotri("matt"), FALSE)
})
