    ...
    if (is.null(x)) {
        ...
        if (.doParse){
            ...
            if (length(.env$cnd) > 0){
                ...
                if (length(.call) == 1) return(.lstC)
                .call <- .call[-1]
                return(lotri(c(list(.lstC),
                               do.call("lotri", .call, envir=envir))))
            }
        }
    }
    if (!is.null(.fullCnd)){
        ...
        if (inherits(.tmp, "list")){
            ...
            return(lotri(c(list(.ret), list(.tmp))))
        }
    }
